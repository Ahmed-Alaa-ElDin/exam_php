$(document).ready(function() {
	...
	var subjectData  = [
						{
							"id":1,
							"name":"English with 2 lines and more asndmore and more",
							"maxClass":5,
							"teachers":[ 
								{
									"id": 1,
	...
	tableThead = '<th class="card-table main"><div class="main">Hours / Days</div></th>';
	tableTbody= '';
	$(".time-table > tbody").html("");
	$.each(data, function(k, v){
		tableThead += '<td class="days-card"><div><div class="sub">'+v.dayName+'</div></div></td>';
		$.each(v.slots, function(key, value) {
			if(k == 0) {
				$(".time-table > tbody").append('<tr row-id="'+key+'"><td class="card-table"><div class="main"><div class="sub">'+value.startTime+' to '+value.endTime+'</div><i class="fas fa-trash"></i></div></td></tr>');
			...
			if(value.isBreak) {
				if(k == 1) {
					$(".time-table tr[row-id='"+key+"']").append('<td colspan="'+data.length+'" class="break-card"><div class="main"><label>Break</label></div></td>');
				}
			} else {
				$(".time-table tr[row-id='"+key+"']").append('<td class="add-card"><div class="main"><div class="sub">Add New</div></div></td>');
			}
			/*if(value.isBreak) {
				if(k == 0) {
					tableTbody += '<td rowspan="'+data.length+'" class="break-card"><label>Break</label></td>';
				}
			} else {
	...
	$('.time-table').delegate('.card-table', 'click', function(){
		rowId = $(this).parent().attr("row-id");
		if($(".time-table > tbody > tr[row-id='"+rowId+"'] > .add-card.active").length + $(".time-table > tbody > tr[row-id='"+rowId+"'] > .subject-card").length == data.length) {
			$(".time-table > tbody > tr[row-id='"+rowId+"'] > .add-card").removeClass("active");
			$(".time-table > tbody > tr[row-id='"+rowId+"'] > .add-card").find(".sub").text("Add New");
			//$(".time-table > tbody > tr[row-id='"+rowId+"'] > td:not(:has(.card-table))").removeClass("active");
			//$(".time-table > tbody > tr[row-id='"+rowId+"'] > td:not(:has(.card-table))").find(".sub").text("Add New");
		} else {
			$(".time-table > tbody > tr[row-id='"+rowId+"'] > .add-card").addClass("active");
			$(".time-table > tbody > tr[row-id='"+rowId+"'] > .add-card").find(".sub").text("Selected");
			//$(".time-table > tbody > tr[row-id='"+rowId+"'] > td:not(:has(.card-table))").addClass("active");
			//$(".time-table > tbody > tr[row-id='"+rowId+"'] > td:not(:has(.card-table))").find(".sub").text("Selected");
		}
	...
	$('.time-table').delegate('.card-table i', 'click', function(e){
		e.stopPropagation();
		rowId = $(this).parent().parent().parent().attr("row-id");
		console.log(rowId);
		$.each($(".time-table tr[row-id='"+rowId+"'] .subject-card"), function() {
