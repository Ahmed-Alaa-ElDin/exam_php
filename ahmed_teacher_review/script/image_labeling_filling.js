$(function() {
    ...
    var json_input = { "answers": [{ "answer_number": 1, "answer_text": "left ear" }, { "answer_number": 2, "answer_text": "nose" }, { "answer_number": 3, "answer_text": "" }, { "answer_number": 6, "answer_text": "mouth" }, { "answer_number": 4, "answer_text": "tail" }, { "answer_number": 5, "answer_text": "leg" }] }
    ...
    setTimeout(function() {
        for (i = 0; i < json_input_teacher.spaces.length; i++) {
            ...
            if (json_input_teacher.spaces[i]["position_x"] < 0.5) {
                newElement = `<div style="position:relative" data-id = "${json_input_teacher.spaces[i]["answer_number"]}" class="student_input_div"><span class="image_tag" style="background-color:${json_input_teacher.spaces[i]["answer_color"]}">${json_input_teacher.spaces[i]["answer_number"]}</span><div  class="student_answer" id="${"fill" + i}" data-id="${json_input_teacher.spaces[i]["answer_number"]}" style="border:2px solid ${json_input_teacher.spaces[i]["answer_color"]}; color:${json_input_teacher.spaces[i]["answer_color"]}; font-weight: bold"></div></div>`
                $(".imageLabelingQuestion .left_group").append(newElement)
            } else {
                newElement = `<div style="position:relative" data-id = "${json_input_teacher.spaces[i]["answer_number"]}" class="student_input_div"><div id="${"fill" + i}" class="student_answer" data-id="${json_input_teacher.spaces[i]["answer_number"]}" style="border:2px solid ${json_input_teacher.spaces[i]["answer_color"]}; color:${json_input_teacher.spaces[i]["answer_color"]}; font-weight: bold"></div><span class="image_tag" style="background-color:${json_input_teacher.spaces[i]["answer_color"]}" >${json_input_teacher.spaces[i]["answer_number"]}</span></div>`
        ...
        setTimeout(() => {
            for (let i = 0; i < json_input.answers.length; i++) {
                if (json_input.answers[i].answer_text != "") {
                    $(`.student_answer[data-id='${json_input.answers[i].answer_number}']`).text(json_input.answers[i].answer_text)
                }
            }
        }, 100);
            ...
            for (let i = 0; i < json_input_teacher.spaces.length; i++) {
                for (let x = 0; x < json_input.answers.length; x++) {
                    if (json_input_teacher.spaces[i].answer_number == json_input.answers[x].answer_number) {
                        if (json_input_teacher.spaces[i].answer_text.toLowerCase() == json_input.answers[x].answer_text.toLowerCase()) {
